...
@Component
public class DataTypesTransformer  {

	private CodeSetTransformer vst = new CodeSetTransformer();

	private final Logger logger = LoggerFactory.getLogger(DataTypesTransformer.class);
        ...
        if(ad.getUses() != null && !ad.getUses().isEmpty()) {
        	...
        	for(PostalAddressUse postalAddressUse : ad.getUses()) {
        		...
        		if(postalAddressUse == PostalAddressUse.PHYS || postalAddressUse == PostalAddressUse.PST) {
        			address.setType(vst.tPostalAddressUse2AddressType(postalAddressUse.toString()));
        			...
        			address.setUse(vst.tPostalAdressUse2AddressUse(postalAddressUse.toString()));
        		}
        	}
        }       
	...
	public Base64BinaryType tBIN2Base64Binary(BIN bin) {
    	...
    	if(bin.getRepresentation().getLiteral()!=null) {
    		...
    		Base64BinaryType base64Binary = new Base64BinaryType();
        	base64Binary.setValue(bin.getRepresentation().getLiteral().getBytes());
        	return base64Binary;
    	}
    	else {
    		return null;
       		...
       		for(CD myCd : cd.getTranslations()) {
				Coding coding = new Coding();
               	...
               	if(myCd.getCodeSystem() != null && !myCd.getCodeSystem().isEmpty()) {
               		coding.setSystem(vst.tOid2Url(myCd.getCodeSystem()));
               		isEmpty = false;
               	}
	...
	public CodeableConcept tCD2CodeableConceptExcludingTranslations(CD cd) {
		...
		if(cd.getCodeSystem() != null && !cd.getCodeSystem().isEmpty()){
			coding.setSystem(vst.tOid2Url(cd.getCodeSystem()));
			isEmpty = false;
		}
	...
	public List<HumanName> tEN2HumanName(EN en) {
		if(en == null || en.isSetNullFlavor())
			return null;

		List<HumanName> myHumanNameList =new ArrayList<>();
		...
		if(en.getUses() != null && !en.getUses().isEmpty()) {
			for(EntityNameUse entityNameUse : en.getUses()) {
				if(entityNameUse != null) {
					myHumanName.setUse(vst.tEntityNameUse2NameUse(entityNameUse.toString()));
				}
			}
		}
		...
		myHumanNameList.add(myHumanName);
		return myHumanNameList;
	...
	public Range tIVL_PQ2Range(IVL_PQ ivlpq){
		...
		if(ivlpq.getLow() != null && !ivlpq.getLow().isSetNullFlavor()){
			range.setLow(tPQ2SimpleQuantity(ivlpq.getLow()));
			
		}
		
		// high -> high
		if(ivlpq.getHigh() != null && !ivlpq.getHigh().isSetNullFlavor()){
			range.setHigh(tPQ2SimpleQuantity(ivlpq.getHigh()));
		}
	...
	public DecimalType tREAL2Decimal(REAL real){
		...
		if(tel.getUses() != null && !tel.getUses().isEmpty()) {
			for(TelecommunicationAddressUse telAddressUse : tel.getUses()) {
				if(telAddressUse != null) {
					contactPoint.setUse(vst.tTelecommunicationAddressUse2ContactPointUse(telAddressUse.toString()));
				}
			}
		}
	...
	public DateType tTS2DateType(TS ts){
	...
	public DateTimeType tTS2DateTime(TS ts) {
		...
		if(dateTime.getPrecision() != Precision.YEAR && dateTime.getPrecision() != TemporalPrecisionEnum.MONTH) {
	 ...
	 * @param classOfReturningObject A FHIR class that determines the initiater for the BaseDateTimeType object the method is to return. Date.class, DateTimeType.class or Instant.class are expected.
	...
	private BaseDateTimeType tTS2BaseDateTime(Object tsObject, Class<?> classOfReturningObject) {
		...
		if(classOfReturningObject == DateType.class) {
			...
			// unexpected situtation
			// caller of this method must have a need of Date, DateTimeType or Instant
			// otherwise, the returning object will be of type Date
			date = new DateType();
		}
