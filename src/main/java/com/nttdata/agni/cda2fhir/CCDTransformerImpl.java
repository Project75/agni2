...
//import ca.uhn.fhir.model.dstu2.composite.Reference;
...
public class CCDTransformerImpl  {
    ...
    public CCDTransformerImpl() {
        ...
        resTransformer = new ResourceTransformer();
     ...
     */
    public Bundle transformDocument2(ClinicalDocument cda, BundleType bundleType, String patientRef, Map<String, String> resourceProfileMap, String xx) {
        ...
        switch (bundleType) {
            case TRANSACTION:
                for(BundleEntryComponent entry : documentBundle.getEntry()) {
                    ...
                    if (entry != null && !entry.getResource().getResourceType().equals("Patient")) {
                        ...
                        if (resourceProfileMap != null) {
                            String profileUri = resourceProfileMap.get(entry.getResource().getResourceType());
                            if (profileUri != null) {
    ...
    public Bundle transformDocument(ClinicalDocument cda) {
        ...
        org.hl7.fhir.dstu3.model.Bundle ccdBundle = resTransformer.tClinicalDocument2Composition(ccd);
        if(ccdBundle != null)
    ...
    private void addRequestToEntry(BundleEntryComponent entry) {
        ...
        //Enumeration<HTTPVerb> post = new Enumeration<HTTPVerb>();
        //post.
		request.setMethod(HTTPVerb.POST);
        request.setUrl(entry.getResource().getResourceType().getPath());
